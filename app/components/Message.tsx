...
export default function Message({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "message-inner space-x-4 flex-grow rounded-md p-4 transition-[opacity, transform] duration-300 delay-250 relative text-sm md:text-base flex text-slate-700 border border-gray-300",
          role === "user" ? "bg-zinc-100" : "bg-white",
          rendered.current
            ? "opacity-100 translate-y-0"
            : "opacity-0 translate-y-7"
        )}
      >
        <div className="message-icon flex items-center justify-center w-10 h-10 min-w-[40px]">
          {role === "user" ? <UserIcon /> : <AssistantIcon />}
        </div>
        <div className="response self-center" ref={messageRef}>
          {thinking ? (
